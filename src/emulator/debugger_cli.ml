...
module Command_interpreter = struct
  ...
  let process history state (request : string) =
    let open Option in
    let open Request in
    let parse request =
      String.capitalize request
      |> Sexp.of_string
      |> t_of_sexp in
    let state' =
      try match request with
        ...
        | "" ->
          LTerm_history.contents history
          |> List.hd_exn
          |> parse
          |> some
          |> state.push_channel;
...
module Repl = struct
  ...
  let make_prompt state =
    let prompt = sprintf "In  [%d]: " state.Command_interpreter.n in
  ...
  let make_output state out =
    ...
    let output = sprintf "Out [%d]: %s" state.n out in
