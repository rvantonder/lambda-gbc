...
let up16 w = Word.to_int w |> ok_exn |> Word.of_int ~width:16
...
let lift (stmt : Z80.Stmt.t) =
  ...
  | `JR,[#Z80.Cond.t as c; `Imm x] ->
    ...
    let true_ = [Bil.(jmp ((var Env.pc) + x16))]
    in
    let false_ = [] in
    (match c with
  ...
  | `LD,[#Z80.Reg16.t as r; `Imm x; `Imm y] ->
    [Bil.(!$ r := int Word.(x@.y))]
  | `LD,[#Z80.Reg8.t as r; `Imm addr] -> [Bil.(!$ r := (int addr))]
  | `LD,[#Z80.Reg8.t as r1; #Z80.Reg8.t as r2] -> [Bil.(!$ r1 := !r2)]
  | `LD,[#Z80.Reg8.t as r1; #Z80.Reg16.t as r2] ->
    [load_from ~dst:!$r1 ~src:!r2]
